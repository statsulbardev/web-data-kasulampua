<div>
    <main id="main" class="main">
        <div class="d-flex pagetitle justify-content-between mb-3 align-items-center">
            <h1>Publikasi</h1>
            <a wire:navigate href="{{ route('backend.publication.create') }}" class="btn" style="background-color: #012970">
                                ...
                                <tr>
                                    ...
                                    <td width="75%">
                                        <div class="d-flex my-2">
                                            <img src="{{ secure_asset('/storage/app/public/' . $publication->picture) }}"
                                                height="150" class="rounded" alt="">
                                            <div class="ms-4">
                                                <small>Judul Publikasi</small>
                                                <p class="fw-bold">{{ $publication->title }}</p>
                                                <a href="{{ $publication->url }}" target="_blank"
                                                    class="btn btn-info rounded text-white"><small>Unduh Publikasi</small></a>
                                            </div>
                                        </div>
                                    </td>
                                    <td width="15%">{{ $publication->created_at->format('d M Y') }}</td>
                                    <td class="d-flex justify-content-between">
                                        <a href="{{ route('backend.publication.edit', $publication->id) }}"
                                            class="btn btn-primary">
                                            <i class="bi bi-pencil"></i>
                                        </a>
                                        <button
                                            wire:click="$set('publicationId', {{ $publication->id }})"
    ...
    <x-inputs.confirmation-modal message="Apakah anda ingin menghapus publikasi ini ?" />
</div>
